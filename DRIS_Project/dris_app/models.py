...
class User(AbstractUser):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.role == 'Authority':
            self.is_staff = True  # Allows admin access

            authority_group, _ = Group.objects.get_or_create(name='Authority')
            self.groups.add(authority_group)

        else:
            self.groups.clear()  # Optional: remove from group if role changes
...
class Skill(models.Model):
    ...
    `description` : str
        A text field that contains a description of the skill, which can be left blank.
    """
